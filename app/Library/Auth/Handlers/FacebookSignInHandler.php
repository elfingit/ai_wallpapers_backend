...
class FacebookSignInHandler extends AbstractSocialSignInHandler
{
    ...
    private function signIn(User $user, FacebookSignInCommand $command): AuthResult
    {
        if (is_null($user->params) || !isset($user->params['facebook_id'])) {
            $params = $user->params ?? [];
            $user->params = array_merge($params, ['facebook_id' => null]);
        }

        if ($user->params['facebook_id'] !== $command->id->value()) {
            $user->params = array_merge($user->params, ['facebook_id' => $command->id->value()]);
            $user->save();
        }
    ...
    private function createUser(FacebookSignInCommand $command): AuthResult
    {
